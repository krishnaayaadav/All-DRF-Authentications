...
# authentication class and permission
...
class GetAPIToken(APIView):
    ...
    def post(self, request, *args, **kwargs):
        # getting user credentials like username and password
        username  = request.data.get('username', None)
        password  = request.data.get('password', None)
        
        # authenticating users credentials like weather any user exist or not with this credentails
        user = authenticate(username=username, password=password)
        
        if user: # if user exist
            #  than  either find or create token
            user_token = Token.objects.get_or_create(user=user)
             
            
            token = {'token': str(user_token)} # just covering it
            return Response(token, status=status.HTTP_200_OK)
        
        else:
            error = {'Invalid Credentials': 'Please provide valid credentials onyl'}
